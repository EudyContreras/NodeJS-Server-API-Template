...
export const setTopFixed = (fixed: boolean) => (dispatch: (action?: any) => any): void => {
	dispatch({ ...fixedTopAction, payload: fixed });
};

export const setBottomFixed = (fixed: boolean) => (dispatch: (action?: any) => any): void => {
	dispatch({ ...fixedBottomAction, payload: fixed });
};

export const setOffsetBottom = (offset: number) => (dispatch: (action?: any) => any): void => {
